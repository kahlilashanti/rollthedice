...
document.querySelector('.btn-roll').addEventListener('click', function(){
  if(gamePlaying){
    ...
    if(dice1 !== 1 && dice2 !== 1) {//if dice is different from 1
      //add score
    ...
    /*if(dice === 6 && lastDice === 6){
      //if player rolls two sixes in a row, the player loses
      //player loses score
      scores[activePlayer] = 0;
      document.querySelector('#score-' + activePlayer).textContent = '0';
      nextPlayer();
    } else if(dice !== 1){//if dice is different from 1
        //add score
        roundScore += dice;
        //then display the score
        document.querySelector('#current-' + activePlayer).textContent = roundScore;
...
document.querySelector('.btn-hold').addEventListener('click', function(){
  if (gamePlaying){
    ...
    if (input){
      // if there is an input then use the string provided in input field
      winningScore = input;
    } else {
      //otherwise use '100'
      winningScore = 100
